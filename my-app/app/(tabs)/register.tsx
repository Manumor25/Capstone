...
export default function RegisterScreen() {
  ...
  // Validar contraseña con mayúscula, minúscula, número y carácter especial
  ...
  const manejarRegistro = async () => {
    ...
    if (!nombres) {
      nuevosErrores.nombres = 'Debes ingresar tu nombre';
    }
    if (!apellidos) {
      nuevosErrores.apellidos = 'Debes ingresar tu apellido';
    }
    if (!validarEmail(correo)) {
      nuevosErrores.correo = 'El correo debe contener un "@"';
    }
    if (!validarContrasena(contrasena)) {
      nuevosErrores.contrasena = 'Debe tener al menos 8 caracteres, incluyendo mayúscula, minúscula, número y carácter especial.';
    }
  ...
  autoComplete="new-password"
      ...
      <View style={styles.checkboxContainer}>
        ...
        <Text style={styles.checkboxLabel}>
          Acepto condiciones de uso y servicio
        </Text>
      </View>
      {errores.aceptaCondiciones ? (
        <Text style={styles.errorText}>{errores.aceptaCondiciones}</Text>
      ) : null}

      <Pressable
        style={[
          styles.button,
          { backgroundColor: aceptaCondiciones ? '#127067' : '#ccc' },
        ]}
